 ...
 * only when the section reaching that end meets it back-to-back — its end pose
 * the reverse of the open end's, the facing join threading seats — so a
 * connection never kinks. A near miss declines the point and falls through to
 * the line and angle snaps, which help align the heading until the approach is
 * tangent.
 *
...
export function resolveSnap(
  ...
  for (const {sectionEnd, pose} of openEnds) {
    ...
    // that section meets the end back-to-back — otherwise the join would kink,
    // which a run never permits. The connecting section reaches the end's
    // position; it qualifies when its far end (B) seats as the reverse of the
    // open end's pose, the facing join threading seats.
