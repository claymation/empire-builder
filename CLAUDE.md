...
- **Variables and parameters are nouns; functions are verbs.** A value is named for the thing it holds, a function for the action it performs (a value-returning function takes the verb, leaving the noun for its result — see above). Destructuring assignments are generally best avoided: one statement mints several new names for values already known by a qualified name (`preview.hover`), and a pattern rename (`hover: hoveredEnd`) mints a synonym. Bind the result to one noun and read its fields.
...
- **When a comment enumerates cases, options, or items, use a Markdown list, not a run-on sentence.** A list of variants (`- \`foo\`: …`) reads and diffs better than the same content strung together with semicolons and dashes.
...
- `at` names a position (`Point`), never a section end or a pose — a section end is an `end` (`onto` for a join target, `railhead` for the selected one), a pose is a `pose`.
