...
Hovering over an inactive railhead (open end) while drawing snaps to the railhead (~5 px magnet) if and only if the sections would join tangent. Clicking to commit joins the sections.
...
Committed track is solid black; previewed track is dashed electric blue; track violating a constraint is red. We use a single stroke to represent the track midline; we do not draw each rail. Mainline track is heavier than siding, spur, and yard track.
